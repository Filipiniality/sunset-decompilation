...
#define FOREACH_SPECIES_IN_KANTO_DEX_ORDER(F) \
    ...
    F(TEDDIURSA) \
    F(URSARING) \
    KANTO_DEX_IF(P_GEN_8_CROSS_EVOS, F(URSALUNA)) \
    F(HOOTHOOT) \
    F(NOCTOWL) \
    F(SEEDOT) \
    F(NUZLEAF) \
    ...
    F(SANDSLASH) \
    F(MURKROW) \
    KANTO_DEX_IF(P_GEN_4_CROSS_EVOS, F(HONCHKROW)) \
    KANTO_DEX_IF(P_GEN_2_CROSS_EVOS, F(CLEFFA)) \
    F(CLEFAIRY) \
    F(CLEFABLE) \
    F(STANTLER) \
    KANTO_DEX_IF(P_GEN_8_CROSS_EVOS, F(WYRDEER)) \
    F(VULPIX) \
    F(NINETALES) \
    ...
    F(XATU) \
    F(YANMA) \
    KANTO_DEX_IF(P_GEN_4_CROSS_EVOS, F(YANMEGA)) \
    F(SNORUNT) \
    F(GLALIE) \
    KANTO_DEX_IF(P_GEN_4_CROSS_EVOS, F(FROSLASS)) \
    F(MANKEY) \
    F(PRIMEAPE) \
    ...
    F(SNEASEL) \
    KANTO_DEX_IF(P_GEN_4_CROSS_EVOS, F(WEAVILE)) \
    F(FARFETCHD) \
    F(WOOPER) \
    F(QUAGSIRE) \
    F(REMORAID) \
    ...
    F(VOLTORB) \
    F(ELECTRODE) \
    KANTO_DEX_IF(P_GEN_4_CROSS_EVOS, F(BONSLY)) \
    F(SUDOWOODO) \
    F(EXEGGCUTE) \
    F(EXEGGUTOR) \
    ...
    F(WEEZING) \
    F(AIPOM) \
    KANTO_DEX_IF(P_GEN_4_CROSS_EVOS, F(AMBIPOM)) \
    F(RHYHORN) \
    F(RHYDON) \
    ...
    F(TOGEPI) \
    F(TOGETIC) \
    KANTO_DEX_IF(P_GEN_4_CROSS_EVOS, F(TOGEKISS)) \
    F(TANGELA) \
    KANTO_DEX_IF(P_GEN_4_CROSS_EVOS, F(TANGROWTH)) \
    F(KANGASKHAN) \
    KANTO_DEX_IF(P_GEN_3_CROSS_EVOS, F(WYNAUT)) \
    F(WOBBUFFET) \
    F(GIRAFARIG) \
    KANTO_DEX_IF(P_GEN_9_CROSS_EVOS, F(FARIGIRAF)) \
    F(OCTILLERY) \
    F(DUNSPARCE) \
    KANTO_DEX_IF(P_GEN_9_CROSS_EVOS, F(DUDUNSPARCE)) \
    F(GLIGAR) \
    KANTO_DEX_IF(P_GEN_4_CROSS_EVOS, F(GLISCOR)) \
    F(SHUCKLE) \
    F(MILTANK) \
    F(HORSEA) \
    F(SEADRA) \
    ...
    F(KIRLIA) \
    F(GARDEVOIR) \
    KANTO_DEX_IF(P_GEN_4_CROSS_EVOS, F(GALLADE)) \
    F(BELDUM) \
    F(METANG) \
    F(METAGROSS) \
    F(OMANYTE) \
