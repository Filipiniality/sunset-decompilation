...
// Stat settings
#define B_BADGE_BOOST               GEN_3 // In Gen4+, Gym Badges no longer boost a Pokémon's stats. (Setting this to GEN_2 does not include the additional boost to the type matching the Gym the Badge is from.)
...
#define B_EXPLOSION_DEFENSE         GEN_3 // In Gen5+, Self-Destruct and Explosion no longer halve the targets' Defense.
...
#define B_UPROAR_TURNS              GEN_3 // In Gen5+, Uproar lasts for 3 turns instead of 2-5.
#define B_UPROAR_IGNORE_SOUNDPROOF  GEN_3 // In Gen5+, Uproar's effects ignore Soundproof.
...
#define B_FAINT_SWITCH_IN           GEN_3 // In Gen4+, sending out a new Pokémon after the previous one fainted happens at the end of the turn. Previously, it would happen after each action.
...
#define B_SKETCH_BANS                   GEN_3 // In Gen9+, Sketch is able to copy less moves than in previous generations.
...
#define B_RAGE_BUILDS                   GEN_3 // In Gen4+, Rage's effect only sets in when it successfully hits. In Gen3, Rage's effect sets in regardless of whether it hits, misses or fails.
...
#define B_OBEDIENCE_MECHANICS           GEN_3 // In PLA onwards (GEN_8 here), obedience restrictions also apply to non-outsider Pokémon, albeit based on their met level rather than their current level.
